/* Formatted on 17/06/2014 18:09:51 (QP5 v5.227.12220.39754) */
...
AS
     SELECT -- 20120912     AG      Semaforo legato a V_MCRES_APP_DEL_ENTE_CENTRALE
           u.val_annomese,
            ...
            SUM (
               CASE
                  WHEN TRUNC (SYSDATE) - TRUNC (dta_inoltro) <= 7 THEN 1
                  ELSE 0
               END)
               val_verde,
            SUM (
               CASE
                  WHEN TRUNC (SYSDATE) - TRUNC (dta_inoltro) BETWEEN 8 AND 30
                  THEN
                     1
                  ELSE
                     0
            ...
            SUM (
               CASE
                  WHEN TRUNC (SYSDATE) - TRUNC (dta_inoltro) > 30 THEN 1
                  ELSE 0
               END)
               val_rosso
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRES_APP_COMUNIC_DELIBERE TO MCRE_USR;
