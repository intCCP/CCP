...
AS
   SELECT alert_41 AS val_alert,
                ...
                AS (SELECT DISTINCT
                      ...
                      FROM (SELECT cod_sndg,
                                   ...
                                   OVER (PARTITION BY cod_sndg
                                      ...
                                      last_5
                              FROM t_mcre0_app_pregiudizievoli p) a
                     WHERE last_5 < 6)
    ...
    WHERE     a.cod_abi_cartolarizzato = x.cod_abi_cartolarizzato
          ...
          AND a.cod_sndg = x.cod_sndg
          AND x.flg_outsourcing = 'Y'
          AND x.flg_target = 'Y'
          AND alert_41 IS NOT NULL
          AND a.cod_sndg = pv.cod_sndg(+)
          AND TRUNC (NVL (PV.DTA_PR_VIS, TO_DATE ('01011000', 'DDMMYYYY'))) <=
                 TRUNC (a.DTA_INS_41);


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRE0_APP_ALERT_PREGIUDIZ TO MCRE_USR;
