/* Formatted on 17/06/2014 18:07:02 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_WRK_ALERT_DEL_DA_CONF
(
   VAL_ALERT,
   COD_SNDG,
   COD_ABI,
...
AS
   SELECT       -- 10/10/2012 aggiunta nvl(dta_ins_delibera, dta_upd_delibera)
          --mm 130521 filtrate le non assegnate (id_utente != -1)
          DISTINCT
          CASE
             WHEN   TRUNC (SYSDATE)
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRE0_WRK_ALERT_DEL_DA_CONF TO MCRE_USR;
