/* Formatted on 17/06/2014 18:15:56 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_APP_GEST_ANOMALIE
(
...
AS
   SELECT ad.cod_abi_cartolarizzato,
     ...
     FROM t_mcre0_app_gest_anomalie ga, t_mcre0_app_all_data_DAY ad
    WHERE     ad.cod_abi_cartolarizzato = ga.cod_abi_cartolarizzato
          AND ad.cod_ndg = ga.cod_ndg
          AND ad.today_flg = '1';


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_APP_GEST_ANOMALIE TO MCRE_USR;
