/* Formatted on 17/06/2014 18:04:39 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_UPD_FIELDS_P1
(
   TODAY_FLG,
...
AS
   SELECT -- 1.0 21/05/2012 Valeria Galli flg_blocco (Commenti con label: VG - CIB/BDT - INIZIO)
         TODAY_FLG,
          COD_ABI_CARTOLARIZZATO,
          COD_NDG,
          COD_ABI_ISTITUTO,
          ...
          CASE WHEN FLG_SOURCE = '0' THEN NVL (COD_STATO, '-1') ELSE 'GB' END
             COD_STATO,
          NULLIF (cod_stato, '-1') AS cod_stato_orig,
          CASE
                ...
                CASE
                   ...
                   THEN
                      (SELECT I.FLG_OUTSOURCING
                        ...
                        WHERE I.COD_ABI = COD_ABI_CARTOLARIZZATO)
                   ELSE
                      FLG_OUTSOURCING
                END
          END
          ...
          SCSB_ACC_SOSTITUZIONI,
          SCSB_UTI_SOSTITUZIONI,
          COD_GRUPPO_SUPER_old         --16 nov: aggiunto per estensione AR/RG
     FROM T_MCRE0_APP_ALL_DATA PARTITION (CCP_P1);


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRE0_APP_UPD_FIELDS_P1 TO MCRE_USR;
