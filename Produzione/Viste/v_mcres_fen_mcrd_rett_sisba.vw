/* Formatted on 17/06/2014 18:12:27 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRES_FEN_MCRD_RETT_SISBA
(
   COD_ABI,
   COD_DIVISIONE,
   VAL_RV,
...
AS
     SELECT                             --- 24/04/2012 ----mese delibere VG-AG
            --- 20/06/2012 ----outer join struttura organizzativa AG
            cod_abi,
            cod_divisione,
            SUM (val_rv) val_rv,
       ...
       FROM (SELECT id_dper,
               ...
               FROM (  SELECT p.id_dper,
                              ...
                              MAX (p.cod_sr_s) cod_sr_s, --sul secondo insieme Ú null
                              MAX (p.cod_sr_cp) cod_sr_cp, --sul primo insieme Ú null
...
GRANT SELECT ON MCRE_OWN.V_MCRES_FEN_MCRD_RETT_SISBA TO MCRE_USR;
