/* Formatted on 17/06/2014 18:17:25 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_CHECK_POS_ISTITUTO
(
   ABI,
   POS_DIR,
   POS
   ...
   SELECT dir.cod_abi_cartolarizzato ABI,
     ...
     FROM (  SELECT a.cod_abi_cartolarizzato, COUNT (*) AS POSIZIONI_DIREZIONE
          ...
          (  SELECT c.cod_abi_cartolarizzato, COUNT (*) AS POSIZIONI
               FROM MCRE_OWN.VTMCRE0_APP_HP_EXCEL c,
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_CHECK_POS_ISTITUTO TO MCRE_USR;
