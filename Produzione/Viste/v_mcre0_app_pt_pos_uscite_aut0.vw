/* Formatted on 17/06/2014 18:02:40 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_PT_POS_USCITE_AUT0
(
...
AS
   SELECT                                         -- v1 17/06/2011 VG: New PCR
    ...
    WHERE     COD_STATO IN ('BO', 'IN', 'XS', 'XC')
   ...
   SELECT DISTINCT
    ...
    WHERE     X.COD_STATO IN ('BO', 'IN', 'XS', 'XC')
          AND X.COD_STATO_PRECEDENTE = 'PT'
          AND X.ID_TRANSIZIONE = 'A'
          AND NVL (X.FLG_OUTSOURCING, 'N') = 'Y';


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRE0_APP_PT_POS_USCITE_AUT0 TO MCRE_USR;
