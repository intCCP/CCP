/* Formatted on 17/06/2014 18:05:39 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_PCR_OLD
(
   TODAY_FLG,
   COD_ABI_ISTITUTO,
...
AS
   SELECT '0' TODAY_FLG,
          ...
          GLGB_UTI_CONSEGNE_DT,
          GLGB_ACC_CONSEGNE_DT
     FROM t_mcre0_dwh_pcr trg
    WHERE NOT EXISTS
                 (SELECT 1
                    FROM ttmcre0_pcr_wrk src   --inclusi quelli da cancellare!
                   WHERE     src.cod_ndg = trg.cod_ndg
                         AND src.cod_abi_cartolarizzato =
                                trg.cod_abi_cartolarizzato);


GRANT SELECT ON MCRE_OWN.V_MCRE0_PCR_OLD TO MCRE_USR;
