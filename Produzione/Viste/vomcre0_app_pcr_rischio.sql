/* Formatted on 17/06/2014 18:13:47 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VOMCRE0_APP_PCR_RISCHIO
(
   COD_ABI_ISTITUTO,
   COD_NDG,
   COD_SNDG,
   COD_GRUPPO_ECONOMICO,
   COD_GRUPPO_LEGAME,
   DESC_ISTITUTO,
   SCSB_ACC_TOT,
   SCSB_UTI_TOT,
   SCSB_TOT_GAR,
   SCSB_DTA_RIFERIMENTO,
   GSB_ACC_TOT,
   GSB_UTI_TOT,
   GSB_TOT_GAR,
   GSB_DTA_RIFERIMENTO,
   GGB_ACC_TOT,
   GGB_UTI_TOT,
   GGB_TOT_GAR,
   GB_DTA_RIFERIMENTO,
   GB_DTA_RIFERIMENTO_CR,
   SCGB_ACC_SIS,
   SCGB_UTI_SIS,
   SCGB_GAR_SIS,
   GEGB_ACC_SIS,
   GEGB_UTI_SIS,
   GEGB_GAR_SIS
)
AS
   SELECT                      -- V1 VG 10/05/2011: Rifatta e Aggiunti dati CR
                                                  -- v2 17/06/2011 VG: New PCR
         X.COD_ABI_ISTITUTO,                       --X.COD_ABI_CARTOLARIZZATO,
         X.COD_NDG,
         X.COD_SNDG,
         G.COD_GRUPPO_ECONOMICO,
         G.COD_GRUPPO_LEGAME,
         I.DESC_ISTITUTO,
         P.SCSB_ACC_TOT,
         P.SCSB_UTI_TOT,
         P.SCSB_TOT_GAR,
         P.SCSB_DTA_RIFERIMENTO,
         DECODE (G.FLG_GRUPPO_ECONOMICO, 1, P.GESB_ACC_TOT, TO_NUMBER (NULL))
            GSB_ACC_TOT,
         DECODE (G.FLG_GRUPPO_ECONOMICO, 1, P.GESB_UTI_TOT, TO_NUMBER (NULL))
            GSB_UTI_TOT,
         DECODE (G.FLG_GRUPPO_ECONOMICO, 1, P.GESB_TOT_GAR, TO_NUMBER (NULL))
            GSB_TOT_GAR,
         DECODE (G.FLG_GRUPPO_ECONOMICO,
                 1, P.GESB_DTA_RIFERIMENTO,
                 TO_DATE (NULL))
            GSB_DTA_RIFERIMENTO,
         (CASE
             WHEN G.FLG_GRUPPO_ECONOMICO = 1
             THEN
                P.GEGB_ACC_CASSA + P.GEGB_ACC_FIRMA
             WHEN G.FLG_GRUPPO_LEGAME = 1
             THEN
                P.GLGB_ACC_CASSA + P.GLGB_ACC_FIRMA
             ELSE
                TO_NUMBER (NULL)
          END)
            GGB_ACC_TOT,
         (CASE
             WHEN G.FLG_GRUPPO_ECONOMICO = 1
             THEN
                P.GEGB_UTI_CASSA + P.GEGB_UTI_FIRMA
             WHEN G.FLG_GRUPPO_LEGAME = 1
             THEN
                P.GLGB_UTI_CASSA + P.GLGB_UTI_FIRMA
             ELSE
                TO_NUMBER (NULL)
          END)
            GGB_UTI_TOT,
         (CASE
             WHEN G.FLG_GRUPPO_ECONOMICO = 1 THEN P.GEGB_TOT_GAR
             WHEN G.FLG_GRUPPO_LEGAME = 1 THEN P.GLGB_TOT_GAR
             ELSE TO_NUMBER (NULL)
          END)
            GGB_TOT_GAR,
         P.SCSB_DTA_RIFERIMENTO GB_DTA_RIFERIMENTO,
         CR.SCGB_DTA_RIF_CR GB_DTA_RIFERIMENTO_CR,
         CR.SCGB_ACC_SIS,
         CR.SCGB_UTI_SIS,
         CR.SCGB_GAR_SIS,
         CR.GEGB_ACC_SIS,
         CR.GEGB_UTI_SIS,
         CR.GEGB_GAR_SIS
    FROM MV_MCRE0_APP_UPD_FIELD X,
         MCRE_OWN.T_MCRE0_APP_ISTITUTI I,
         T_MCRE0_APP_PCR P,
         T_MCRE0_APP_CR CR,
         T_MCRE0_APP_FILE_GUIDA G
   WHERE     X.COD_ABI_CARTOLARIZZATO = P.COD_ABI_CARTOLARIZZATO(+)
         AND X.COD_NDG = P.COD_NDG(+)
         AND X.COD_ABI_CARTOLARIZZATO = CR.COD_ABI_CARTOLARIZZATO(+)
         AND X.COD_NDG = CR.COD_NDG(+)
         AND X.COD_ABI_CARTOLARIZZATO = G.COD_ABI_CARTOLARIZZATO
         AND X.COD_NDG = G.COD_NDG
         AND X.COD_ABI_ISTITUTO = I.COD_ABI(+)
         AND X.COD_STATO = 'PT';


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.VOMCRE0_APP_PCR_RISCHIO TO MCRE_USR;
