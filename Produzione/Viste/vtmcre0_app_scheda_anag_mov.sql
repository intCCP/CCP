/* Formatted on 17/06/2014 18:17:08 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_APP_SCHEDA_ANAG_MOV
(
...
AS
   SELECT                                    -- V1 11/05/2011 VG: controllo SO
          ...
          TFIN.COD_NDG,
          TFIN.COD_PERCORSO,
          -- substr(tms_ini,0,13), substr(tms_fine,0,13), substr(TFIN.tms,0,13) ttms,
          ...
          CASE
             WHEN     tms_ini IS NOT NULL
                  ...
                  AND (    SUBSTR (TFIN.TMS, 0, 13) >
                              SUBSTR (rs.TMS_INI, 0, 13)
                       AND SUBSTR (TFIN.TMS, 0, 13) <=
                              NVL (SUBSTR (rs.TMS_FINE, 0, 13),
                                   '9999-12-31.00'))
             THEN
          ...
          CASE
             WHEN     tms_ini IS NOT NULL
                  --                                          AND TFIN.DTA_DECORRENZA_STATO > rs.DTA_DECORRENZA_STATO THEN
                  AND SUBSTR (TFIN.TMS, 0, 13) <
                         NVL (SUBSTR (rs.TMS_FINE, 0, 13), '9999-12-31.00')
             THEN
          ...
          CASE
             WHEN     tms_ini IS NOT NULL
                  --                                          AND TFIN.DTA_DECORRENZA_STATO > rs.DTA_DECORRENZA_STATO THEN
                  AND SUBSTR (TFIN.TMS, 0, 13) <
                         NVL (SUBSTR (rs.TMS_FINE, 0, 13), '9999-12-31.00') --rs.DTA_DECORRENZA_STATO
             THEN
             ...
             FROM (SELECT M.COD_ABI_CARTOLARIZZATO,
                     ...
                     /*FROM t_mcre0_app_mople m
                                                                                                                                                               ...
                                                                                                                                                               ON (u.id_utente = m.id_utente)
                                                                                                                                                                                                                                         WHERE 1                                             = 1 */
                     FROM VTMCRE0_APP_UPD_FIELDS_PALL M,
                   ...
                   SELECT PTMP.COD_ABI_CARTOLARIZZATO,
                    ...
                    WHERE     1 = 1
                          ...
                          AND SUBSTR (PTMP.TMS, 0, 13) BETWEEN SUBSTR (
                                                                  rs.TMS_INI(+),
                                                                  0,
                                                                  13)
                                                           AND NVL (
    ...
    WHERE     NOT (TFIN.COD_STATO = '-1' AND TFIN.COD_STATO_PRECEDENTE = '-1')
          ...
          AND SUBSTR (TFIN.TMS, 0, 13) BETWEEN SUBSTR (rs.TMS_INI(+), 0, 13)
                                           AND NVL (
                                                  SUBSTR (rs.TMS_FINE(+),
                                                          0,
                                                          13),
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_APP_SCHEDA_ANAG_MOV TO MCRE_USR;
