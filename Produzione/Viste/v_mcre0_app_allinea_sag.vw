/* Formatted on 17/06/2014 18:01:01 (QP5 v5.227.12220.39754) */
   ...
   SELECT /*+ordered no_parallel(s) no_parallel(q)*/
    ...
    FROM (SELECT X.COD_ABI_CARTOLARIZZATO,
                 ...
                 NVL (X.COD_GRUPPO_SUPER_OLD, X.COD_GRUPPO_SUPER)
                    COD_GRUPPO_SUPER,             -- 08/04/14 modificato campo
                 S.COD_SAG,
           ...
           WHERE     Q.ALERT_2 IS NOT NULL
                 ...
                 AND NVL (P.COD_GRUPPO_SUPER_OLD, P.COD_GRUPPO_SUPER) =
                        NVL (X.COD_GRUPPO_SUPER_OLD, X.COD_GRUPPO_SUPER) --modificato filtro
                 AND X.COD_SNDG = S.COD_SNDG(+)
                 AND S.FLG_CONFERMA(+) = 'S')
   WHERE VAL_CNT > 1;


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRE0_APP_ALLINEA_SAG TO MCRE_USR;
