/* Formatted on 17/06/2014 18:17:36 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_WRK_ALERT_INS_PIANO
(
   VAL_ALERT,
   DTA_INS,
   COD_ABI,
...
AS
   SELECT DISTINCT
     ...
     FROM (SELECT d12.cod_abi,
                  d12.cod_ndg,
                  d12.cod_sndg,
                  d12.cod_protocollo_delibera,
                  -- LIVELLO 2 DELIBERE STIME DS
                  d12.cod_stato,
                  s11.cod_rapporto,
                  s11.min_dta
             FROM (SELECT d11.cod_abi,
                     ...
                     FROM (SELECT d1.cod_abi,
                                  ...
                                  ad.cod_stato,
                                  RANK ()
                                  OVER (PARTITION BY d1.cod_abi, d1.cod_ndg
                                        ORDER BY
                                           d1.dta_conferma_delibera DESC,
                            ...
                            WHERE     ad.cod_abi_cartolarizzato = d1.cod_abi
                                  ...
                                  AND d1.cod_fase_pacchetto NOT IN
                                         ('ANM', 'ANA', 'CNF', 'ULT')
                                  -- ANCORA IN FAS EDI ELABORAZIONE
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_WRK_ALERT_INS_PIANO TO MCRE_USR;
