/* Formatted on 17/06/2014 18:09:10 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_WRK_CHECK_LAST_LOAD_M
(
   COD_DIPENDENZA,
   ABI,
   FLUSSO,
               ...
               FROM (SELECT q.id_flusso,
                    ...
                    (SELECT cod_abi,
                       ...
                       FROM t_mcrei_wrk_anag_flussi,
                            (SELECT MAX (id_dper - 1) periodo_contr
                               FROM t_mcrei_wrk_acquisizione)
                      WHERE FLUSSO LIKE 'PROPOSTE_MOPLE%') f
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCREI_WRK_CHECK_LAST_LOAD_M TO MCRE_USR;
