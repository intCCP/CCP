/* Formatted on 17/06/2014 18:05:20 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_GB_GESTIONE
(
   TODAY_FLG,
   FLG_ACTIVE,
   FLG_SOURCE,
   ID_DPERFG,
   ID_DPERMO,
   COD_ABI_ISTITUTO,
   COD_ABI_CARTOLARIZZATO,
   COD_NDG,
   COD_SNDG,
   COD_GRUPPO_ECONOMICO,
   COD_GRUPPO_LEGAME,
   FLG_GRUPPO_ECONOMICO,
   FLG_GRUPPO_LEGAME,
   FLG_SINGOLO,
   FLG_CONDIVISO,
   FLG_SOMMA,
   FLG_OUTSOURCING,
   COD_FILIALE,
   COD_STRUTTURA_COMPETENTE,
   COD_STATO,
   COD_STATO_ORIG,
   DTA_DECORRENZA_STATO,
   DTA_SCADENZA_STATO,
   COD_STATO_PRECEDENTE,
   DTA_DECORRENZA_STATO_PRE,
   DTA_INTERCETTAMENTO,
   COD_TIPO_INGRESSO,
   COD_CAUSALE_INGRESSO,
   COD_PERCORSO,
   COD_PROCESSO,
   DTA_PROCESSO,
   COD_PROCESSO_PRE,
   COD_MACROSTATO,
   DTA_DEC_MACROSTATO,
   COD_RAMO_CALCOLATO,
   COD_COMPARTO_CALCOLATO,
   COD_COMPARTO_CALCOLATO_PRE,
   COD_GRUPPO_SUPER,
   COD_GRUPPO_SUPER_OLD,
   FLG_RIPORTAFOGLIATO,
   DTA_LAST_RIPORTAF,
   FLG_PROPOSTO_ASSEGNATO,
   DTA_UTENTE_ASSEGNATO,
   COD_COMPARTO_ASSEGNATO,
   ID_UTENTE,
   ID_UTENTE_PRE,
   COD_SERVIZIO,
   DTA_SERVIZIO,
   COD_COMPARTO,
   COD_MATR_ASSEGNATORE,
   COD_SEZIONE_PREASSEGNATA,
   ID_UTENTE_PREASSEGNATO,
   COD_COMPARTO_PREASSEGNATO,
   ID_STATO_POSIZIONE,
   COD_CLIENTE_ESTESO,
   ID_CLIENTE_ESTESO,
   DESC_ANAG_GESTORE_MKT,
   COD_GESTORE_MKT,
   COD_TIPO_PORTAFOGLIO,
   FLG_GESTIONE_ESTERNA,
   VAL_PERC_DECURTAZIONE,
   COD_COMPARTO_HOST,
   ID_TRANSIZIONE,
   COD_RAMO_HOST,
   COD_MATR_RISCHIO,
   COD_UO_RISCHIO,
   COD_DISP_RISCHIO,
   DTA_INS,
   DTA_UPD,
   COD_OPERATORE_INS_UPD,
   COD_FILIALE_GB,
   COD_PROCESSO_CALCOLATO_GB,
   COD_PROCESSO_CALCOLATO,
   COD_MACROSTATO_PROPOSTO_GB,
   COD_MACROSTATO_PROPOSTO,
   DTA_INS_GB,
   FLG_STATO_GB,
   FLG_BLOCCO
)
AS
   SELECT /*+full(gb) hash(w,gb)*/
         '1' today_flg,
          '1' flg_active,
          '1' flg_source,
          w.id_dperfg,
          w.id_dpermo,
          gb.cod_abi_istituto,
          gb.cod_abi_cartolarizzato,
          gb.cod_ndg,
          gb.cod_sndg,
          CASE
             WHEN cod_gruppo_economico = '-1' THEN NULL
             ELSE cod_gruppo_economico
          END
             cod_gruppo_economico,
          CASE
             WHEN cod_gruppo_legame = '-1' THEN NULL
             ELSE cod_gruppo_legame
          END
             cod_gruppo_legame,
          flg_gruppo_economico,
          flg_gruppo_legame,
          flg_singolo,
          flg_condiviso,
          flg_somma,
          flg_outsourcing,
          gb.cod_filiale AS cod_filiale,
          w.cod_struttura_competente,
          --          --stati
          'GB' cod_stato,
          NULLIF (w.cod_stato, '-1') AS cod_stato_orig,
          dta_stato AS dta_decorrenza_stato,
          NULL AS dta_scadenza_stato,
          --          --
          w.cod_stato AS cod_stato_precedente,
          w.dta_decorrenza_stato AS dta_decorrenza_stato_pre,
          dta_intercettamento,
          cod_tipo_ingresso,
          cod_causale_ingresso,
          cod_percorso,
          gb.cod_processo_calcolato AS cod_processo,
          gb.dta_ins AS dta_processo,
          w.cod_processo AS cod_processo_pre,
          'GB' AS cod_macrostato,
          gb.dta_ins AS dta_dec_macrostato,
          cod_ramo_calcolato,
          cod_comparto_calcolato,
          cod_comparto_calcolato_pre,
          cod_gruppo_super,
          cod_gruppo_super_old,
          flg_riportafogliato,
          dta_last_riportaf,
          flg_proposto_assegnato,
          dta_utente_assegnato,
          cod_comparto_assegnato,
          id_utente,
          id_utente_pre,
          cod_servizio,
          dta_servizio,
          NVL (NVL (cod_comparto_assegnato, cod_comparto_calcolato), '#')
             cod_comparto,
          cod_matr_assegnatore,
          cod_sezione_preassegnata,
          id_utente_preassegnato,
          cod_comparto_preassegnato,
          id_stato_posizione,
          cod_cliente_esteso,
          id_cliente_esteso,
          desc_anag_gestore_mkt,
          cod_gestore_mkt,
          cod_tipo_portafoglio,
          flg_gestione_esterna,
          val_perc_decurtazione,
          cod_comparto_host,
          id_transizione,
          cod_ramo_host,
          cod_matr_rischio,
          cod_uo_rischio,
          cod_disp_rischio,
          gb.dta_ins AS dta_ins,
          w.dta_upd,
          cod_operatore_ins_upd,
          --GB
          gb.cod_filiale cod_filiale_gb,
          gb.cod_processo_calcolato cod_processo_calcolato_gb,
          gb.cod_processo_calcolato,
          gb.cod_macrostato_proposto cod_macrostato_proposto_gb,
          gb.cod_macrostato_proposto,
          gb.dta_ins dta_ins_gb,
          flg_stato flg_stato_gb,
          -- CIB/BDT
          w.FLG_BLOCCO
     FROM t_mcre0_all_data w, t_mcre0_app_gb_gestione gb
    WHERE     w.cod_abi_cartolarizzato(+) = gb.cod_abi_cartolarizzato
          AND w.cod_ndg(+) = gb.cod_ndg
          AND gb.flg_stato = 1;


GRANT SELECT ON MCRE_OWN.V_MCRE0_GB_GESTIONE TO MCRE_USR;
