/* Formatted on 17/06/2014 18:17:47 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCREI_APP_DATI_DETT_CHIUS
(
   COD_ABI,
   COD_NDG,
   ...
   SELECT                                --0221 introdotta nuova pcr_rapp_aggr
         DISTINCT
          ...
          NVL (d.val_rdv_qc_ante_delib, 0) + NVL (d.val_rdv_pregr_fi, 0)
             AS val_totale_rettifiche_pregr,          -----campo da verificare
          d.val_rinuncia_deliberata AS val_rinuncia_prog_totale,  ---10 aprile
          a.cod_tipo_gestione AS cod_gestione,
          do.desc_dominio AS desc_gestione,
          d.cod_causa_chius_delibera AS cod_causale_chiusura, ---CONTIENE LA CAUSALE PER LE DELIB DI CH.INCAGLIO O DI CH.RISTRUTT
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCREI_APP_DATI_DETT_CHIUS TO MCRE_USR;
