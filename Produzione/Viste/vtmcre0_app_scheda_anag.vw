/* Formatted on 17/06/2014 18:17:03 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_APP_SCHEDA_ANAG
(
...
AS
 ...
 --  21/05/2012 Valeria Galli pregiudizievoli (Commenti con label:   VG - CIB/BDT - INIZIO)
         ...
         CASE                           --0320 distinguo tra direzione e altro
            ...
            ELSE
               CASE
                  WHEN f.cod_stato IN ('OC', 'OP', 'IN') THEN f.dta_scadenza_stato
                  ELSE TO_DATE (NULL)
               END
         END
         ...
         ris.desc_tipo_ristr,                                        ---26 oct
         ris.desc_intento_ristr,
         ris.dta_efficacia_ristr,
         --19/09/2012
         f.cod_percorso
    -------------------- VG - CIB/BDT - FINE --------------------
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_APP_SCHEDA_ANAG TO MCRE_USR;
