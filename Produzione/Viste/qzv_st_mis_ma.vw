/* Formatted on 17/06/2014 17:59:09 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.QZV_ST_MIS_MA
(
   COD_SRC,
   ID_DPER,
   DTA_COMPETENZA,
     ...
     SELECT 'MA' COD_SRC,
       ...
       FROM mcre_own.QZT_FT_MIS_MON_INC_SOF x,
            (SELECT F.ID_DPER COD_ANNOMESE,
              ...
              WHERE COD_FLT_TMP = 'A' AND ORD_FLT_TMP = 1 AND cod_src = 'MM' --AND f.id_dper = substr(SYS_CONTEXT ('userenv', 'client_info'),1,6)
                                                                            ) f
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.QZV_ST_MIS_MA TO MCRE_USR;
