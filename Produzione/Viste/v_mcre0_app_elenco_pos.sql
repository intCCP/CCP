/* Formatted on 17/06/2014 18:01:26 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_ELENCO_POS
(
   COD_COMPARTO,
   DESC_COMPARTO,
   DESC_ISTITUTO,
...
AS
   SELECT      --0709 : esposta dta_decorrenza come in scheda anag per IN e SO
          --0118 : variate aggregazioni pratiche per performance
          cod_comparto,
          desc_comparto,
          desc_istituto,
          cod_struttura_competente_dv,
                           ...
                           FROM (SELECT DISTINCT
                                 ...
                                 SELECT DISTINCT
                                  ...
                                  WHERE     dta_fine_stato !=
                                               TO_DATE ('31/12/9999',
                                                        'DD/MM/YYYY')
                                        AND flg_attiva = 1 --GROUP BY     cod_abi, cod_ndg
                                                          ) pr --GROUP BY     pr.cod_abi, pr.cod_ndg
                                                              ) pr
          ...
          f.cod_matricola,
          --16 nov per estensione AR/RG
          f.id_referente,
          flg_outsourcing,
          cod_gruppo_super,
          --16 nov per estensione AR/RG
          cod_gruppo_super_old,
          f.cod_livello                          --16 nov per estensione AR/RG
     FROM v_mcre0_app_upd_fields f
    WHERE flg_stato_chk = '1';


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCRE0_APP_ELENCO_POS TO MCRE_USR;
