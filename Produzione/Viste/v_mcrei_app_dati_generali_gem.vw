/* Formatted on 17/06/2014 18:07:36 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_DATI_GENERALI_GEM
(
   COD_NDG,
...
AS
   SELECT /*+ no_parallel (g)*/
         d.cod_ndg,
          ...
          DECODE (d.cod_tipo_pacchetto, 'B', 'Y', 'N') flg_pacchetto_batch,
          d.val_anno_proposta,
          ...
          CASE
             WHEN NVL (
                     NVL (NULLIF (g.DESC_ISTITUTO, '#'),
                          NULLIF (g.DESC_ISTITUTO, '#')),
                     NULLIF (g.DESC_ISTITUTO, '#'))
                     IS NOT NULL
             THEN
                NVL (
             ...
             ELSE
                (SELECT o.desc_struttura_competente
                  ...
                  WHERE     o.cod_abi_istituto = g.cod_abi_cartolarizzato
                        AND O.COD_COMPARTO =
                               NVL (g.cod_comparto_assegnato,
                                    g.cod_comparto_calcolato))
          END
             desc_uo_proponente,
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCREI_APP_DATI_GENERALI_GEM TO MCRE_USR;
