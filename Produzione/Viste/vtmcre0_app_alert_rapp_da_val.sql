/* Formatted on 17/06/2014 18:15:06 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_APP_ALERT_RAPP_DA_VAL
(
   VAL_ALERT,
   ...
   SELECT pos.alert AS val_alert,
     ...
     FROM (SELECT DISTINCT st_max.cod_abi,
            ...
            WHERE     st_max.cod_abi = st.cod_abi
                  ...
                  AND d.flg_attiva = '1'
                  AND D.COD_MICROTIPOLOGIA_DELIB IN ('RV', 'T0', 'A4', 'IM')) pos,
          vtmcre0_app_upd_fields ad
    WHERE     pos.cod_abi = ad.cod_abi_cartolarizzato
          AND pos.cod_ndg = ad.cod_ndg;


GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_APP_ALERT_RAPP_DA_VAL TO MCRE_USR;
