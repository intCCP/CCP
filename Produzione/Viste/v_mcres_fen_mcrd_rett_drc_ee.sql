/* Formatted on 17/06/2014 18:12:26 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRES_FEN_MCRD_RETT_DRC_EE
(
   ID_DPER,
   COD_ABI,
   COD_DIVISIONE,
...
AS
     SELECT                             --- 24/04/2012 ----mese delibere VG-AG
       ...
       FROM (  SELECT ee.id_dper,
                ...
                WHERE     0 = 0
                      AND (ee.cod_stato_ini = 'S') --cosidero solo le posizioni che a inizio mese sono a sofferenza (se stato_ini != 'S' e stato_fin= 'S' allora la posizione Ú entrata a sofferenza nel mese considerato e di_cui_drc = 0)
...
GRANT SELECT ON MCRE_OWN.V_MCRES_FEN_MCRD_RETT_DRC_EE TO MCRE_USR;
