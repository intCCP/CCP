/* Formatted on 17/06/2014 18:17:16 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCRE0_APP_STATI_AMMINISTR
(
   COD_MATRICOLA,
   COD_ABI,
...
AS
   SELECT u.cod_matricola,
          ...
          CASE
             WHEN     P.COD_STATO IN ('IN', 'RI', 'SO')
                  AND P.COD_STATO IS NOT NULL
             THEN
          ...
          NULL cod_indicatore
     FROM t_mcre0_app_all_data_DAY P,
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCRE0_APP_STATI_AMMINISTR TO MCRE_USR;
