/* Formatted on 17/06/2014 18:17:51 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VTMCREI_APP_PRATICHE
(
   COD_ABI,
...
AS
   SELECT DISTINCT
          ...
          --x le rdv dico se ho fatto proroga implicita
          CASE
             WHEN d."COD_MICROTIPOLOGIA_DELIB" IN
                     ('RV', 'T4', 'A0', 'IM', 'IF')
             THEN  --uso la regola standard, usata anche nelle classificazioni
     ...
     FROM t_mcrei_app_delibere d,
          ...
          (SELECT DISTINCT
                  ...
                  -- 15FEB: ESTRAGGO ULTIMA CLASSIFICAZIONE SE CE NE SONO PIu DI UNA CAUSA IMPIANTO
                  OVER (
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON MCRE_OWN.VTMCREI_APP_PRATICHE TO MCRE_USR;
