/* Formatted on 17/06/2014 18:07:51 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_DETT_DATI_GEN_GEM
(
   COD_ABI,
   COD_NDG,
   COD_PROT_DELIBERA,
...
AS
   SELECT -- 23/10/2012 I.Gueorguieva Aggiunte distinct nelle sottoselect sulle stime, che moltiplicavano i record per rapporto
         cod_abi,
          cod_ndg,
          cod_prot_delibera,
          cod_prot_pacchetto,
          ...
          val_tot_rett_progr_parere,
          val_rett_attuale_cassa,
          (NVL (val_esp_cassa_qta_cap, 0) - NVL (val_rett_cassa_qta_cap, 0))
             AS val_esp_netta_post_delibera,                        --22 marzo
          (NVL (val_rett_cassa_qta_cap, 0) - NVL (val_rdv_pregr_cassa, 0))
     ...
     FROM ( (SELECT DISTINCT
                    ...
                    DECODE (
                       ...
                       NVL (d2.val_rdv_rapp_operativi, s2.rdv_operativi))
                    ...
                    D.DTA_LAST_UPD_DELIBERA AS DTA_AGGIORNAMENTO,
                    d.desc_note
               FROM v_mcrei_app_dett_rapporti_gem r,
                    t_mcrei_app_delibere d,
                    t_mcrei_app_delibere d2,                         ---24 OTT
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK, MERGE VIEW ON MCRE_OWN.V_MCREI_APP_DETT_DATI_GEN_GEM TO MCRE_USR;
