CREATE OR REPLACE PACKAGE BODY MCRE_OWN."PKG_MCREI_WEB_UTILITIES_DAPORT" IS
  /*
  ...
  6.54       15/11/2012       i.gueorguieva       crea_pacchetto e aggiungi_microtipologia creano delibere con flg_no_delibera per le posizioni di area o regione non assegnate al gestore corrente (o quello per cui si sta operando)
   6.55       22/11/2012      d'errico            aggiunto salvataggio dta_Scadenza in fase di conferma di una rv (popola_dett_Rapp_rv)

*/
  /*PER IL LOG:PKG_MCREI_AUDIT.LOG_APP NELLE FUNZIONI DEL PORTALE*/
  ...
  BEGIN
    ...
    UPDATE t_mcrei_app_delibere d
       SET val_perc_dubbio_esito = p_val_perc_dubbio_esito
     WHERE d.cod_protocollo_pacchetto = p_proto_pacchetto
       AND d.cod_abi = p_cod_abi
       AND d.cod_ndg = p_cod_ndg
       AND d.cod_protocollo_delibera = p_proto_delibera;
  ...
  BEGIN
    ...
    RETURN PKG_MCREI_WEB_UTILITIES_DAPORT.const_esito_ok;
  EXCEPTION
    WHEN OTHERS THEN
      pkg_mcrei_audit.log_app(PKG_MCREI_WEB_UTILITIES_DAPORT.c_package ||
                              '.POPOLA_MAN_RI_URG_CAU_RIF',
                              3,
                              SQLCODE,
                              SQLERRM,
                              ...
                              p_proto_pacchetto,
                              NULL);
      RETURN PKG_MCREI_WEB_UTILITIES_DAPORT.const_esito_ko;
  ...
  BEGIN
    ...
    THEN
      v_capo_gruppo := PKG_MCREI_WEB_UTILITIES_DAPORT.get_capogruppo(p_cod_protocollo_pacchetto);
    ELSE
      v_capo_gruppo := '-';
    END IF;
  ...
  BEGIN
    ...
    v_proto_pacchetto := ctrl_esist_pacc_aperto(p_sndg);
    ...
    ELSE
      ...
      EXCEPTION
        WHEN no_data_found THEN
          ...
          SELECT dta_creazione_pacchetto
            INTO v_dta_creaz
            FROM t_mcrei_app_delibere d
           WHERE d.cod_protocollo_pacchetto = v_proto_pacchetto
             AND rownum = 1;
  ...
  BEGIN
    ...
    THEN
      ...
      LOOP
        ...
        V_CREA_DELIBERA:=1;
--        V_IS_DI_REG_O_AR:= PKG_MCREI_WEB_UTILITIES_DAPORT_EVOL.FNC_IS_AR_RG_LIV_AREA(v_rec.cod_comparto);
--        IF V_IS_DI_REG_O_AR THEN
--            V_CREA_DELIBERA:= 0;
--            IF TO_CHAR(V_REC.ID_UTENTE) = P_UTENTE_INS THEN
--                V_CREA_DELIBERA:= 1;
--            END IF;
--        END IF;
--        IF V_CREA_DELIBERA = 0 then
--            p_note:=p_note||CHR(10)||'Attenzione:   Creata delibera con flg_no_delibera = 1 per '||v_rec.cod_abi_cartolarizzato||
            ...
            SELECT   val_rdv_qc_progressiva,val_esp_netta_post_delib,cod_protocollo_delibera,
                ...
                FROM (SELECT nvl(s.val_rdv_qc_progressiva, 0) val_rdv_qc_progressiva,
                       ...
                       WHERE s.cod_abi = v_rec.cod_abi_cartolarizzato
                         ...
                         AND flg_attiva = '1'
                       ORDER BY --.dta_upd_fase_delibera  DESC,
                                s.val_num_progr_delibera DESC,
                                s.dta_conferma_delibera  DESC)
               WHERE rownum <= 1;
            EXCEPTION
              WHEN no_data_found THEN
                val_rdv_qc_progr   := 0;
...
--        IF V_IS_DI_REG_O_AR THEN
--            V_CREA_DELIBERA:= 0;
--            IF TO_CHAR(V_REC.ID_UTENTE) = P_UTENTE_INS THEN  V_CREA_DELIBERA:= 1;END IF;
--        END IF;
--        IF V_CREA_DELIBERA = 0 then
--            p_note:=p_note||CHR(10)||'Creata delibera con flg_no_delibera = 1 per la posizione'||v_rec.cod_abi_cartolarizzato||
--            ' '||v_rec.cod_ndg||' che e'' di area o regione'||
--            CHR(10)||' e non e'' assegnata al gestore corrente (o quello per cui si sta operando).'||chr(10);
--        END IF;
      ...
      --0808 se l'ultima delibera contabilizzata S CT recupero la somma degli stralci come perdita iniziale
        ...
        INSERT INTO t_mcrei_app_delibere d
          ...
          (SELECT to_number(to_char(trunc(SYSDATE), 'YYYYMMDD')),
                  ...
                  CASE
                    WHEN (nvl(pc.scsb_acc_tot + pc.scsb_uti_tot, 0) = 0 AND
                         p_microtipolog != 'CH') THEN --v5.3
                     1
                     WHEN V_CREA_DELIBERA = 0 THEN
                     1
...
--      END IF;
  ...
  -- %CD 7 NOV 2011
  -- %PARAM P_SNDG: SUPER NDG
FUNCTION ctrl_esist_pacc_aperto(p_sndg IN VARCHAR2) RETURN VARCHAR2 IS
  ...
  FUNCTION rimuovi_microtipologia(p_prot_pacch IN VARCHAR2,
                                  p_microtipol IN VARCHAR2) RETURN NUMBER IS
  ...
  BEGIN
    ...
    THEN
      p_note := 'DELETE MICROTIPOLIGIA O INTERO PACCHETTO su ' ||
      ...
      pkg_mcrei_audit.log_app(c_nome,
                              ...
                              p_note,
                              NULL);
      --SE NON VIENE PASSATA LA MICROTIPOLOGIA, SI CANCELLA L'INTERO PACCHETTO
    END IF;
    ...
    p_note := 'loop per retrocessione wf su tutte le microtipologie del pacchetto ' ||
              p_prot_pacch;
  ...
  BEGIN
    ...
    THEN
      v_flg_conferma := 'Y';
     IF p_microtipologia NOT IN ('CI', 'CS') THEN
    ...
    CASE
      WHEN v_flg_conferma = 'Y' THEN
        ...
        IF p_microtipologia IN ('CI', 'CS', 'CZ')
        THEN
          --p_note := 'MICROTIPOLIGIA IN ''CI'',''CS'',''CZ''';
          ...
          INSERT INTO t_mcrei_app_posiz_con_classif
            ...
            (SELECT f.cod_sndg,
              ...
              WHERE de.cod_abi = f.cod_abi_cartolarizzato
                ...
                AND de.cod_protocollo_delibera = p_prot_delib
                AND f.today_flg = '1'
                   -- CLASSIFICAZIONI ANCORA IN ATTESA DI PRATICA, OPPURE CON STATO_DELIBERA CONF O IN ATTESA
  ...
  FUNCTION annulla_pacchetto(p_proto_pacch IN t_mcrei_app_delibere. cod_protocollo_pacchetto%TYPE,
                             p_flg_tipo    VARCHAR2) RETURN NUMBER IS
  ...
  FUNCTION get_capogruppo(p_proto_pacch IN t_mcrei_app_delibere. cod_protocollo_pacchetto%TYPE)
    RETURN VARCHAR2 IS
    c_nome CONSTANT VARCHAR2(100) := PKG_MCREI_WEB_UTILITIES_DAPORT.c_package ||
                                     '.GET_CAPOGRUPPO';
    p_note    t_mcrei_wrk_audit_applicativo.note%TYPE;
  ...
  BEGIN
    ...
    ON (t.cod_abi = s.cod_abi_cartolarizzato AND t.cod_ndg = s.cod_ndg AND t. cod_protocollo_pacchetto = p_proto_pacchetto AND t.cod_protocollo_delibera = p_proto_delibera AND t.flg_attiva = '1' AND t.cod_macrotipologia_delib = 'GE' AND t.cod_microtipologia_delib NOT IN('CI',
                                                                                                                                                                                                                                                                               'RV') AND t.cod_fase_pacchetto NOT IN('ANA',
                                                                                                                                                                                                                                                                                                                     'ANM'))
    WHEN MATCHED THEN
      UPDATE
  ...
  BEGIN
    ...
    THEN
      ...
      THEN
        ...
        IF v_tot = PKG_MCREI_WEB_UTILITIES_DAPORT.const_esito_ko
          ...
          INSERT INTO t_mcrei_app_stime st
            ...
            SELECT DISTINCT decode(p_flg_archiviazione,
                            ...
                            */ --> nuova logica:
                            ...
                            substr(p_proto_delibera, 1, 8)) || '-' --11 giu
              FROM t_mcrei_app_pcr_rapporti p
             WHERE p.cod_abi = p_cod_abi
               AND p.cod_ndg = p_cod_ndg
               AND p.cod_rapporto = p_cod_rapporto
 ...
 FUNCTION popola_man_dett_dati_gen_rv(p_cod_abi            VARCHAR2,
  ...
  BEGIN
    ...
    BEGIN
      SELECT to_char(trunc((((nvl(mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia(p_cod_abi,
                                                                                               p_cod_ndg,
                                                                                               'CT'),
                                  0) + nvl(mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia(p_cod_abi,
                                                                                                           p_cod_ndg,
                                                                                                           'CM'),
                                              0) +
                           nvl(mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia(p_cod_abi,
                                                                                               p_cod_ndg,
                                                                                               'CO'),
                                  0)) /
                           ((nvl(val_uti_cassa_scsb, val_esp_lorda)) +
                           nvl(mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia(p_cod_abi,
                                                                                               p_cod_ndg,
                                                                                               'CT'),
                                  0)))) * 100))
    ...
    UPDATE t_mcrei_app_delibere d
       SET d.val_rinuncia_capitale         = p_val_rinuncia_capitale,
           ...
           d.val_rinuncia_totale           = nvl(d.val_rinuncia_deliberata,
                                             ...
                                             nvl(p_val_rinuncia_capitale, 0) +
                                             nvl(p_val_rinuncia_mora, 0),
           d.val_perc_perd_rm              = v_perc_rinuncia ----percentuale di rinuncia
     WHERE d.cod_abi = p_cod_abi
       AND d.cod_ndg = p_cod_ndg
  ...
  BEGIN
    ...
    BEGIN
    --mm0808 aggiorno fino a CM, poi non ricalcolo pi¿ (andrebbe ricevuta in input dalle viste..
--      SELECT to_char(trunc((((nvl(mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia(p_cod_abi,
--                                                                                               p_cod_ndg,
--                                                                                               'CT'),
--                                  0) + nvl(mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia(p_cod_abi,
 ...
 WHERE cod_abi = p_cod_abi
   ...
   AND cod_protocollo_delibera = p_cod_protocollo_delibera;
    ...
    UPDATE (SELECT --= DELIBERE =--
             ...
             p.scsb_uti_cassa,
             val_perc_perd_rm
            -- AS perc_rinuncia--percentuale di rinuncia
              FROM t_mcrei_app_delibere      d,
                   t_mcrei_app_pcr_rapp_aggr p
             WHERE d.cod_abi = p.cod_abi_cartolarizzato(+)
               AND d.cod_ndg = p.cod_ndg(+)
       ...
       SET val_accordato          = PP.scsb_acc_tot,
           ...
           val_uti_tot_scsb       = PP.scsb_uti_tot,
    ...
    BEGIN
      INSERT INTO t_mcrei_app_delibere d
        (id_dper,
         ...
         val_rinuncia_deliberata,
         val_imp_perdita, --0807
         val_stralcio_senza_accantonam )--23/08)
        (SELECT to_number(to_char(trunc(SYSDATE), 'YYYYMMDD')),
                g.cod_sndg,
                p_cod_abi,
                p_cod_ndg,
  ...
  /*FUNCTION crea_pacchetto_extra_del(p_sndg       IN VARCHAR2,
                                    p_abi        IN VARCHAR2,
                                    p_ndg        IN VARCHAR2,
                                    p_utente_ins IN VARCHAR2) RETURN VARCHAR2 IS

    c_nome CONSTANT VARCHAR2(100) := c_package ||
                                     '.CREA_PACCHETTO_EXTRA_DEL';
    v_prot             t_mcrei_app_delibere.cod_protocollo_pacchetto%TYPE;
    p_note             t_mcrei_wrk_audit_applicativo.note%TYPE;
    v_progre_delib     NUMBER;
    val_rdv_qc_progr   NUMBER := 0;
    ...
    val_esp_netta_post NUMBER := 0;
    v_imp_perdita      NUMBER := 0;
    v_delibera_pre     VARCHAR2(25);

  BEGIN

    p_note := 'CREA_PACCHETTO_EXTRA_DEL: ' || p_sndg;
    ...
    BEGIN
      SELECT val_rdv_qc_progressiva,
        ...
        FROM (SELECT nvl(s.val_rdv_qc_progressiva, 0) val_rdv_qc_progressiva,
                     ...
                     cod_protocollo_delibera,
                     nvl(val_rdv_progr_fi, 0) AS val_rdv_progr_fi,
                     nvl(val_imp_perdita, 0) AS val_imp_perdita --6.0
                FROM t_mcrei_app_delibere s
               WHERE s.cod_abi = p_abi
                 AND s.cod_ndg = p_ndg
                 AND s.cod_fase_delibera IN ('AD', 'CO', 'CT', 'NA') ---aggiunto CT il 11/4, NA il 24/4
                 AND s.flg_no_delibera = 0
              -- AND s.dta_conferma_delibera IS NOT NULL
    ...
    INSERT INTO t_mcrei_app_delibere d
      ...
      SELECT to_number(to_char(trunc(SYSDATE), 'YYYYMMDD')),
             ...
             SYSDATE,
             SYSDATE,
             'X', ---TIPO PACCHETTO EXTRA DELIBERA
             p_utente_ins,
             val_rdv_qc_progr, -- VAL_RDV_QC_PROGRESSIVA
             val_esp_netta_post, -- VAL_ESP_NETTA_POST_DELIB
             val_rdv_qc_progr AS val_rdv_qc_ante_delib,
             val_rdv_qc_progr AS val_esp_netta_ante_delib, --9 maggio
             0 AS val_rdv_qc_deliberata,
             seq_mcrei_prog_del.nextval,
             nvl(v_rdv_progr_fi, 0),
             nvl(v_rdv_progr_fi, 0),
             v_imp_perdita
       ...
       WHERE g.cod_abi_cartolarizzato = p_abi
         AND g.cod_ndg = p_ndg
         AND g.cod_sndg = p_sndg;

    pkg_mcrei_audit.log_app(c_nome,
                            pkg_mcrei_audit.c_debug,
                            SQLCODE,
                            'proto_pacchetto ' || v_prot,
                            p_note,
                            'EXTRA_DEL');
  ...
  FUNCTION popola_dett_ristr(p_cod_abi            IN t_mcrei_app_delibere.cod_abi%TYPE,
    ...
    v_ret                   NUMBER := PKG_MCREI_WEB_UTILITIES_DAPORT.const_esito_ok;
  BEGIN
    p_params := 'abi = ' || p_cod_abi || ', ndg = ' || p_cod_ndg ||
                ', proto_delibera = ' || p_proto_delib ||
                ', proto_pacchetto = ' || p_proto_pacchetto ||
    ...
    IF v_ret = PKG_MCREI_WEB_UTILITIES_DAPORT.const_esito_ok
    THEN
...
END PKG_MCREI_WEB_UTILITIES_DAPORT;
/


CREATE SYNONYM MCRE_APP.PKG_MCREI_WEB_UTILITIES_DAPORT FOR MCRE_OWN.PKG_MCREI_WEB_UTILITIES_DAPORT;


CREATE SYNONYM MCRE_USR.PKG_MCREI_WEB_UTILITIES_DAPORT FOR MCRE_OWN.PKG_MCREI_WEB_UTILITIES_DAPORT;


GRANT EXECUTE, DEBUG ON MCRE_OWN.PKG_MCREI_WEB_UTILITIES_DAPORT TO MCRE_USR;
