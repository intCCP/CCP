CREATE OR REPLACE PACKAGE BODY MCRE_OWN."PKG_MCRE0_PCR2" AS
/******************************************************************************
NAME: PKG_MCRE0_PCR2
...
EXCEPTION
WHEN OTHERS THEN
LOG_CARICAMENTI('fnc_calcolo_mau',SQLCODE,'UPDATE MAU PCR - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
ROLLBACK;
RETURN KO;
END;
...
EXCEPTION
WHEN OTHERS THEN
LOG_CARICAMENTI('fnc_calcolo_mau',SQLCODE,'GEGB uniforme - t_mcre0_app_pcr - ge: '||GE.COD_GRUPPO_ECONOMICO||' - SQLERRM='||SQLERRM);
END;
END LOOP;
commit;
END;
...
EXCEPTION
WHEN OTHERS THEN
LOG_CARICAMENTI('fnc_calcolo_mau',SQLCODE,'generale - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
ROLLBACK;
RETURN KO;
end;
...
exception
when others then
LOG_CARICAMENTI('fnc_clean_no_pcr',SQLCODE,'UPDATE NO PCR - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
rollback;
return ko;
END;
...
exception
WHEN OTHERS THEN
LOG_CARICAMENTI('fnc_clean_no_pcr',SQLCODE,'UPDATE USCITI dal gruppo economico - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
rollback;
RETURN KO;
end;
...
exception
WHEN OTHERS THEN
LOG_CARICAMENTI('fnc_clean_no_pcr',SQLCODE,'UPDATE USCITI da legame - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
rollback;
RETURN KO;
end;
...
exception
when others then
LOG_CARICAMENTI('fnc_clean_no_pcr',SQLCODE,'DELETE NO PCR - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
rollback;
end;
...
exception
when others then
log_caricamenti('fnc_clean_no_pcr',SQLCODE,'CLEAN PCR - t_mcre0_app_pcr - SQLERRM='||SQLERRM);
return ko;
end;
...
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'SCSB - START');
val_ok := fnc_load_pcr_scsb_merge(V_COD_LOG );
IF(VAL_OK=KO)THEN
raise e_invalid_pcr;
END IF;
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'GESB - START');
val_ok := fnc_load_pcr_gesb_merge(V_COD_LOG);
if(val_ok=ko)then
raise e_invalid_pcr;
end if;
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'GB - START');
val_ok := fnc_load_pcr_gb_merge(V_COD_LOG);
IF(VAL_OK=KO)THEN
raise e_invalid_pcr;
END IF;
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'CLEAN - START');
val_ok := fnc_clean_no_pcr(V_COD_LOG);
IF(VAL_OK=KO)THEN
raise e_invalid_pcr;
END IF;
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'MAU - START');
VAL_OK := FNC_CALCOLO_MAU(V_COD_LOG);
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'END');
IF(VAL_OK=KO)THEN
raise e_invalid_pcr;
END IF;

--v2.0 - ripristino la partizione attiva
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'UPDATE PCR FLG_TAODAY - START');
Update t_mcre0_app_pcr
set today_flg = case when (cod_abi_cartolarizzato, cod_ndg) in (select cod_abi_cartolarizzato, cod_ndg
from t_mcre0_app_mople PARTITION (CCP_P1))
then '1'
else '0'
end;
commit;
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_DEBUG,SQLCODE,SQLERRM,'UPDATE PCR FLG_TAODAY - END');
...
commit;
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_ERROR,SQLCODE,SQLERRM,'e_invalid_pcr - SQLERRM='||SQLERRM);
return ok;

WHEN OTHERS THEN
PKG_MCRE0_AUDIT.LOG_ETL(V_COD_LOG,'PKG_MCRE0_PCR2.FNC_LOAD_PCR',PKG_MCRE0_AUDIT.C_ERROR,SQLCODE,SQLERRM,'GENERALE - SQLERRM='||SQLERRM);
return ko;
end;
...
WHEN MATCHED THEN
UPDATE SET
a.SCSB_UTI_CASSA     = b.SCSB_UTI_CASSA     ,
...
MERGE /*+ FULL (a) */ INTO mcre_own.t_mcre0_app_pcr a
