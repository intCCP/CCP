/* Formatted on 21/07/2014 18:40:46 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_REP_DUBBIO_ESIT_IG
(
   DUBBIO_ESITO
)
AS
     ...
     FROM (SELECT *
             FROM (SELECT (CASE
                           ...
                           END)
                             AS COD_ABI,
                          COD_NDG,
                          DTA_REF AS DTA_DECORRENZA_STIMA,
                          DTA_DELIBERA_IT AS DTA_DELIBERA,
                          DTA_SCAD_DELIB_IT AS DTA_SCADENZA,
                          DESC_MOTIVAZIONE_PERC_RISK AS DESC_OSSERVAZIONI,
                          VAL_IMPORTO_DELIBERA_IT AS IMP_DELIB,
