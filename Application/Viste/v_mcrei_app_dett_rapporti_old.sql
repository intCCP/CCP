/* Formatted on 21/07/2014 18:40:02 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_DETT_RAPPORTI_OLD
(
   COD_ABI,
   COD_NDG,
   DTA_STIMA,
...
AS
   SELECT DISTINCT
     ...
     FROM (( (SELECT DISTINCT
                     ...
                     --MODIFICATO IL 13 MARZO PER CAMBIO LOGICA
                     'Y' AS flg_storico,
                     ---- 23 MAGGIO   : C'è ALMENO SE STESSO NELLO STORICO
                     DECODE (nat.cod_natura,  '01', 'BR',  '02', 'MLT')
                        AS val_intervallo,
                     ----24 ottobre: AD --> rdv su rapp operativi è quella sui rapporti di tipo R di cassa
...
(p.val_accordato_delib)*/
            ...
            (SELECT DISTINCT
                    ...
                    --MODIFICATO IL 13 MARZO PER CAMBIO LOGICA
                    NULL AS val_stima_di_rec,
                    --MODIFICATO IL 13 MARZO PER CAMBIO LOGICA
                    'Y' AS flg_storico,
                    ---- 23 MAGGIO   : SI PUNTA COMUNQUE ALLO STORICO, AL MAX è VUOTO
                    DECODE (nat.cod_natura,  '01', 'BR',  '02', 'MLT')
                       AS val_intervallo,
