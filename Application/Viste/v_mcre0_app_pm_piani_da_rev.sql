...
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_PM_PIANI_DA_REV
(
   ID_UTENTE,
   ID_REFERENTE,
   COD_COMPARTO,
   ...
   SELECT a.ID_UTENTE,
     ...
     FROM V_MCRE0_APP_UPD_FIELDS a,
          (SELECT DISTINCT
                  ...
                  CASE
                     WHEN GREATEST (
                             NVL (dta_presa_visione_modifica,
                                  TO_DATE ('01/01/1900', 'DD/MM/YYYY')),
                             DTA_PIANO_VALIDATO) > SYSDATE - 30
                     THEN
                        1
                     ELSE
                        0
            ...
            WHERE     NVL (FLG_PIANO_ANNULLATO, 'N') = 'N'
                  AND ID_WORKFLOW IN (15, 20)
                  AND dta_scadenza > SYSDATE) b
