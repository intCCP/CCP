/* Formatted on 21/07/2014 18:44:34 (QP5 v5.227.12220.39754) */
   ...
   UNION ALL
     ...
     SELECT SUBSTR (s.cod_autorizzazione, 1, 4) cod_sap_societa,
            ...
            || RPAD (
                  ...
                  ' ')                                           -- conto GOGE
            || RPAD (' ', 24, ' ')                             -- elemento WBS
            || LPAD (SUM (f.val_imponibile_iva) * 100, 13, '0') -- imponibile IVA
            || 'SZ' --'SA'                                                                                                   -- codice SAP IVA
            || LPAD (0, 8, '0') --lpad(sum(f.val_aliquota_iva) * 100, 8, '0')                                                 -- aliquota IVA
            || LPAD (0, 13, '0') --lpad(sum(f.val_importo_iva) * 100, 13, '0')                                                -- importo IVA
            || LPAD (SUM (f.val_importo_pos) * 100, 13, '0') -- importo posizione
            || RPAD ('0', 13, '0')                                 -- quantità
            || RPAD ('0', 13, '0')                             -- prezzo netto
            || RPAD (' ', 13, ' ')                                -- numero EM
            || RPAD (' ', 99, ' ')                                   -- filler
   ...
   ***
   ...
   SELECT SUBSTR (s.cod_autorizzazione, 1, 4) cod_sap_societa,
          ...
          || RPAD (' ', 69, ' ')                                     -- filler
             line
     FROM spese s
    WHERE 0 = 0 AND s.cod_tipo_autorizzazione IN ('1', '6')
   UNION ALL
   ---
   SELECT SUBSTR (s.cod_autorizzazione, 1, 4) cod_sap_societa,
          5 val_ordine_tipo_record,
          ...
          || RPAD (NVL (s.cod_causa_divisa, ' '), 3, ' ')            -- divisa
          || LPAD (NVL (s.val_tot_imp * 100, '0'), 20, '0') -- totale imponibile
          || LPAD (0, 20, '0') --lpad(nvl(s.val_tot_iva * 100, '0') , 20,'0')                                               -- totale IVA
          || LPAD (NVL (s.val_aliq_contrprev * 100, '0'), 20, '0') -- aliquota CPA
          || LPAD (NVL (s.val_imp_contrprev * 100, '0'), 20, '0') -- importo CPA
          || LPAD (NVL (s.val_aliq_ritacc * 100, '0'), 20, '0') -- aliquota RA
          || LPAD (NVL (s.val_imp_ritacc * 100, '0'), 20, '0')   -- importo RA
          ...
          || RPAD (' ', 69, ' ')                                     -- filler
             line
     FROM spese s
    WHERE 0 = 0 AND s.cod_tipo_autorizzazione = '5'
   ---
   /*************************************
   ***
   ***  6- PROTOCOLLI
