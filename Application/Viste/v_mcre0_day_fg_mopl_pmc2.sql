/* Formatted on 21/07/2014 18:36:52 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_DAY_FG_MOPL_PMC2
(
   TODAY_FLG,
   ID_DPER,
   ID_DPERFG,
     ...
     FROM (SELECT a.*,
                  CASE
                     ...
                     THEN
                           cod_comparto_calcolato
                        || SUBSTR (
                                 SUBSTR (cod_gruppo_super_1, 1, 7)
                              || CASE
                                    WHEN cod_stato = 'PM'
                                    THEN
                                       'P'
                                    WHEN cod_abi_cartolarizzato = '10637'
                                    THEN
                                       'M'
                                    ELSE
                                       '0'
                                 END
                              || SUBSTR (cod_gruppo_super_1, 9, 15),
                              LENGTH (cod_comparto_calcolato) + 1,
                              20)                                           --
                     ELSE
                           SUBSTR (cod_gruppo_super_1, 1, 7)
                        || CASE
                              WHEN cod_stato = 'PM' THEN 'P'
                              WHEN cod_abi_cartolarizzato = '10637' THEN 'M'
                              ELSE '0'
                           END
                        || SUBSTR (cod_gruppo_super_1, 9, 15)
                  END
             ...
             FROM (SELECT f.*,
                          ...
                          NVL (
                             ...
                             cod_comparto_calcolato,
                          MIN (
                             NVL (m.cod_comparto_host, '999999'))
                          OVER (
                             PARTITION BY    SUBSTR (cod_gruppo_super_1,
                                                     1,
                                                     7)
                                          || CASE
                                                WHEN m.cod_stato = 'PM'
