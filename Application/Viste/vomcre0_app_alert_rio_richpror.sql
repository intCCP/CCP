/* Formatted on 21/07/2014 18:45:56 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.VOMCRE0_APP_ALERT_RIO_RICHPROR
(
   VAL_ALERT,
   VAL_ORDINE_COLORE,
   DTA_INS_ALERT,
...
AS
   SELECT                                -- v1 20/04/2011 VG: FLG_ABI_LAVORATO
          ...
          (SELECT DECODE (
                     ...
                     DECODE (alert_19,
                             'A', val_arancio,
                             DECODE (alert_19, 'R', val_rosso, NULL)))
             FROM t_mcre0_app_alert
            WHERE id_alert = 19)
             val_ordine_colore,
          dta_ins_19 dta_ins_alert,
          a.cod_sndg,
