...
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_ELENCO_DELIB_STR1
(
   ID_DPER,
...
AS
   (SELECT d.id_dper,
      ...
      FROM t_mcrei_app_delibere_storico d
           ...
           INNER JOIN t_mcrei_app_pratiche p
              ON D.COD_ABI = P.COD_ABI AND D.COD_NDG = P.COD_NDG
           INNER JOIN
    ...
    UNION ALL
    SELECT TO_NUMBER (TO_CHAR (TRUNC (SYSDATE, 'MM'), 'YYYYMMDD')) id_dper,
           f.cod_struttura_competente,
           f.cod_comparto,
           f.desc_comparto,
           d.cod_protocollo_delibera,
     ...
     WHERE     d.flg_no_delibera = 0
           ...
           AND D.COD_FASE_DELIBERA != 'AN'
           AND TO_NUMBER (
                  TO_CHAR (TRUNC (d.dta_last_upd_delibera), 'YYYYMM')) =
                  TO_NUMBER (TO_CHAR (TRUNC (SYSDATE), 'YYYYMM')));
