/* Formatted on 21/07/2014 18:32:24 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_ALERT_LOAD_TB
(
   ...
   SELECT ------------------------------ VERSIONE SYSTEM ----------------------------
     ...
     FROM (SELECT m.cod_abi_cartolarizzato,
                  ...
                  CASE
                     ...
                     THEN
                        (SELECT CASE
                                   WHEN (x.num_giorni_sconfino_rap BETWEEN 60
                                                                       AND 150)
                                   THEN
                                      'V'
                        ...
                        (SELECT CASE
                                   ...
                                   WHEN     c.val_livello IN
                                               ('DIVISIONE',
                                                'REGIONE',
                                                'AREA')
                                        --modifica per includere aree e regioni
                                        AND x.cod_stato IN ('OC', 'IN', 'RS')
                                   THEN
                                      CASE
                  ...
                  CASE
                     ...
                     THEN
                        (SELECT CASE
                                   WHEN (    t_mcre0_app_comparti.val_livello IN
                                                ('DIVISIONE',
                                                 'REGIONE',
                                                 'AREA')
                                         --modifica per regioni e aree
             ...
             FROM (SELECT a.cod_abi_cartolarizzato,
                          ...
                          NVL (
                             (SELECT DISTINCT 1
                               ...
                               WHERE     a.cod_abi_istituto =
                                     ...
                                     --m.m. 130531 incluse pef 440
                                     AND NVL (cod_fase_pef, 0) IN (440, 321)),
                             0)
                             alert_3,
                          ------------------ ALERT_4 ---------------------
                          (CASE
                              WHEN     a.cod_stato NOT IN
                                   ...
                                   AND DECODE (
                                          b.val_livello,
                                          'DIREZIONE', (  a.flg_gruppo_economico
                                                        + a.flg_gruppo_legame),
                                          0) = 0
                                   AND a.id_utente != -1
                              THEN
                          ...
                          (CASE
                              WHEN     a.cod_macrostato IN
                                   ...
                                   AND EXISTS
                                          (SELECT DISTINCT 1
                                             FROM T_MCRE0_APP_GEST_PRATICA_FASI Z --t_mcre0_app_rio_azioni z                            <--MOD
                                            WHERE     Z.COD_ABI_CARTOLARIZZATO =
                                                  ...
                                                  AND DECODE (
                                                         ...
                                                         'RS', 'IN',
                                                         a.cod_macrostato) =
                                                         z.cod_macrostato --mod
                                                  AND z.cod_ndg = a.cod_ndg
                                                  AND z.dta_scadenza <
                                                         TRUNC (SYSDATE)
                                                  AND Z.FLG_COMPLETATA != 'Y' --z.flg_status != 'C'                                        --mod
                                                  AND z.flg_delete = 'N')
                              THEN
                                 1
                              ELSE
                          ...
                          (CASE
                              WHEN     a.cod_macrostato IN
                                          ('RIO', 'IN', 'RS', 'SC')  --('RIO')
                                   --in system ('RIO', 'IN', 'RS', 'SC')
                                   -- estensione a IN, RS e SC
                                   AND NOT EXISTS
                                              (SELECT DISTINCT 1
                                                 FROM V_MCRE0_APP_GEST_PRATICA_FASI v --v_mcre0_app_rio_azioni v                              <--MOD
                                                WHERE (   (    NVL (
                                                       ...
                                                       OR     (    NVL (
                                                               ...
                                                               AND V.COD_NDG =
                                                                      A.COD_NDG)
                                                          AND V.COD_MACROSTATO =
                                                                 A.COD_MACROSTATO) --AND v.cod_tipologia_pratica IS NOT NULL
                                                                                  --modifica per lo sdoppiamento dell'alert.
                                                                                  --18/10: commentata modifica sdoppiamneto!
                                           )
                              THEN
                                 1
                              ELSE
                          ...
                          (CASE
                              WHEN (   (a.cod_stato = 'GB')
                                    ...
                                    OR (    a.cod_macrostato IN
                                               ('RIO', 'IN', 'RS', 'SC') --= 'RIO'
                                        AND (   EXISTS
                                                   (SELECT 1
                                                      FROM T_MCRE0_APP_GEST_POSIZIONE g --t_mcre0_app_rio_gestione g                             <--MOD
                                                     WHERE     G.COD_ABI_CARTOLARIZZATO =
                                                                  A.COD_ABI_CARTOLARIZZATO
                                                           AND g.COD_MACROSTATO =
                                                                  A.COD_MACROSTATO
                                                           AND g.cod_macrostato =
                                                                  a.cod_macrostato
                                                           AND g.cod_ndg =
                                          ...
                                          (SELECT DISTINCT 1
                                            ...
                                            WHERE     Z.COD_ABI_CARTOLARIZZATO =
                                                  ...
                                                  AND (  dta_scadenza
                                                       - TRUNC (SYSDATE)) >=
                                                         0
                                                  AND Z.FLG_COMPLETATA != 'Y' --FLG_STATUS != 'C'
                                                                             )
                              THEN
                                 1
                              ELSE
                              ...
                              WHEN     (   (    b.val_livello = 'DIREZIONE'
                                        ...
                                        OR (    b.val_livello IN
                                                   ('DIVISIONE',
                                                    'REGIONE',
                                                    'AREA')
                                            --modifica per aree regioni
                                            AND (   a.cod_stato = 'OC'
                                                 OR (    a.cod_stato IN
                                                            ('IN', 'RS')
                                                     AND   A.DTA_SCADENZA_STATO
                                                         - TRUNC (SYSDATE) <=
                                                            30))))
                                   AND NOT EXISTS
                                              (SELECT DISTINCT 1
                          ...
                          ------------------ ALERT_40 ---------------------
                          (CASE
                              WHEN ( (    b.val_livello IN
                                             ('DIVISIONE', 'AREA', 'REGIONE')
                                      --modifica per regioni e aree
                                      AND a.id_utente IS NOT NULL
                                      AND a.cod_stato IN ('OC', 'IN', 'RS')
                                      AND EXISTS
                                             (SELECT DISTINCT 1
                                                FROM t_mcre0_app_rio_proroghe g
                                               WHERE     g.cod_abi_cartolarizzato =
                                                     ...
                                                     AND g.flg_storico = 0
                                                     --                   AND g.id_utente =
                                                     --                      a.id_utente  --14/02 Commentata condizione di join sull'utente. Da ripristinare.
                                                     AND (   NVL (
                                                                g.val_num_proroghe,
                                                                0) = 0
                                                          OR (    NVL (
