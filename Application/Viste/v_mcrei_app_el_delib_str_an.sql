/* Formatted on 21/07/2014 18:40:09 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_EL_DELIB_STR_AN
(
   ID_DPER,
...
AS
   (SELECT d.id_dper,
           ...
           NVL (DTA_ANNULLO, DTA_UPD_FASE_DELIBERA) AS DTA_ANNULLO,
           D.COD_MATRICOLA_ANNULLO,
           D.COD_OPERA_COME_ANNULLO,
           D.DESC_NOTE_DELIBERE_ANNULLATE
      FROM T_MCREI_APP_DELIBERE_STORICO D
     WHERE     D.COD_FASE_DELIBERA = 'AN'
           AND d.desc_note_delibere_annullate IS NOT NULL
    UNION ALL
    SELECT TO_NUMBER (
              TO_CHAR (TRUNC (DTA_UPD_FASE_DELIBERA, 'MM'), 'YYYYMMDD'))
              id_dper,
     ...
     WHERE     d.flg_no_delibera = 0
           ...
           AND d.desc_note_delibere_annullate IS NOT NULL
           AND NVL (TRUNC (DTA_ANNULLO), TRUNC (DTA_UPD_FASE_DELIBERA)) >=
                  TRUNC (SYSDATE, 'MM'));
