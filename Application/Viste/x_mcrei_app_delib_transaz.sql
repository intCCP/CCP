...
CREATE OR REPLACE FORCE VIEW MCRE_OWN.X_MCREI_APP_DELIB_TRANSAZ
(
   COD_SNDG,
   COD_ABI,
...
AS
   SELECT DISTINCT
          ...
          DECODE (
             (  NVL ( (NVL (val_uti_cassa_scsb, val_esp_lorda)), 0)
              + NVL (
                   mcre_own.pkg_mcrei_gest_delibere.fnc_mcrei_get_last_rinuncia (
                      d.cod_abi,
                      d.cod_ndg,
     ...
     FROM t_mcre0_app_all_data a,
          (SELECT d.*, r.scsb_uti_tot, scsb_uti_cassa
             FROM t_mcrei_app_pcr_rapp_aggr r, t_mcrei_app_delibere d
