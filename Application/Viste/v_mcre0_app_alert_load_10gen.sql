/* Formatted on 21/07/2014 18:32:09 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_ALERT_LOAD_10GEN
(
...
AS
   SELECT h."COD_ABI_CARTOLARIZZATO",
          ...
          h."ALERT_20",
          h."ALERT_21",
          --          h."ALERT_22", h."ALERT_23", h."ALERT_24",
          h."ALERT_25",
          h."ALERT_40",
          h."ALERT_41",
          h."ALERT_45",
          ...
          h.dta_upd_20,
          h.dta_upd_21,
          --          h.dta_upd_22, h.dta_upd_23, h.dta_upd_24,
          h.dta_upd_25,
          h.dta_upd_40,
          h.dta_upd_41,
          h.dta_upd_45,
          ...
          ***/
          ...
          DECODE (
             ...
             0)
             flg_worse_upd_21,
          --          DECODE (SIGN (  DECODE (alert_22_old, 'V', 1, 'A', 2, 'R', 3, 4)
          --                        - DECODE (alert_22, 'V', 1, 'A', 2, 'R', 3, 0)
          --                       ),
          --                  -1, 1,
          --                  0
          --                 ) flg_worse_upd_22,
          --          DECODE (SIGN (  DECODE (alert_23_old, 'V', 1, 'A', 2, 'R', 3, 4)
          ...
          DECODE (
             SIGN (
                  DECODE (alert_45_old,  'V', 1,  'A', 2,  'R', 3,  4)
 ...
 --                                                'AREA') --10/01/13 commento per escludere l'estensione a aree e regioni
                  ...
                  CASE
                     ...
                     THEN
                        CASE
                           WHEN (  TRUNC (SYSDATE)
                                 - TRUNC (NVL (dta_ins_45, SYSDATE)) BETWEEN 0
                                                                         AND 7)
                           THEN
                              'V'
                           WHEN (  TRUNC (SYSDATE)
                                 - TRUNC (NVL (dta_ins_45, SYSDATE)) BETWEEN 8
                                                                         AND 15)
                           THEN
                              'A'
                           WHEN (  TRUNC (SYSDATE)
                  ...
                  --                  NVL (dta_ins_22, SYSDATE) dta_ins_22, SYSDATE dta_upd_22,
                  --                  tmp.alert_23 alert_23_old,
                  --                  NVL (dta_ins_23, SYSDATE) dta_ins_23, SYSDATE dta_upd_23,
                  ...
                  NVL (dta_ins_45, SYSDATE) dta_ins_45,
                  SYSDATE dta_upd_45
                                              ...
                                              (SELECT DISTINCT 1
                                                ...
                                                WHERE     (   (    NVL (
                                                           ...
                                                           OR (    NVL (
                                                               ...
                                                               AND v.cod_ndg =
                                                                      a.cod_ndg))
                                                      AND v.cod_tipologia_pratica
                                                             IS NOT NULL --modifica per lo sdoppiamento dell'alert.
                                                                        )
                              THEN
                                 1
                              ELSE
                                        ...
                                        AND (   EXISTS
                                                   (SELECT 1
                                                     ...
                                                     WHERE     g.cod_abi_cartolarizzato =
                                                           ...
                                                           AND g.flg_workflow =
                                                                  3
                                                           AND g.cod_macrostato_destinazione
                                                                  IS NOT NULL)
                                             OR EXISTS
                                                   (SELECT 1
