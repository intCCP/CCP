/* Formatted on 21/07/2014 18:32:28 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCRE0_APP_ALERT_LOADI
(
...
AS
   SELECT  ------------------- versione produzione ---------------------------
         h."COD_ABI_CARTOLARIZZATO",
          ...
          h."ALERT_21",
          h."ALERT_22",
          h."ALERT_23",                              --          h."ALERT_24",
          h."ALERT_25",
          h."ALERT_40",
          h."ALERT_41",                 --                       h."ALERT_45",
          h.alert_1_old,
          h.alert_2_old,
          h.alert_3_old,
          h.alert_4_old,
          ...
          DECODE (h.alert_41, NULL, NULL, h.dta_ins_41) dta_ins_41, --          DECODE (h.alert_45, NULL, NULL, h.dta_ins_45) dta_ins_45,
          h.dta_upd_1,
          h.dta_upd_2,
          h.dta_upd_3,
          h.dta_upd_4,
...
***/
 ...
 --                                                'AREA') --10/01/13 commento per escludere l'estensione a aree e regioni
                                        AND x.cod_stato IN ('OC', 'IN', 'RS') --v14.03.13
                                   THEN
                                      CASE
 ...
 --                                                'AREA') --10/01/13 commento per escludere l'estensione a aree e regioni
                     ...
                     THEN
                        (SELECT CASE
                                   WHEN (    t_mcre0_app_comparti.val_livello =
                                                'DIVISIONE' --                                        IN ('DIVISIONE', 'REGIONE',
                                         --                                                      'AREA') --10/01/13 commento per escludere l'estensione a aree e regioni
                                         AND x.cod_stato IN
                                                ('OC', 'IN', 'RS')
             ...
             FROM (SELECT a.cod_abi_cartolarizzato,
                          ...
                          ***/
                          ...
                          ------------------ ALERT_7 ---------------------
                          (CASE
                              WHEN     a.cod_macrostato IN ('RIO') --IN('RIO', 'IN', 'RS', 'SC')-- 14/01/13 estensione a IN, RS e SC commentata
                                   AND NOT EXISTS
                                              (SELECT DISTINCT 1
                                                   ...
                                                   (SELECT 1
                                                     ...
                                                     WHERE     g.cod_abi_cartolarizzato =
                                                           ...
                                                           AND g.cod_ndg =
                                                                  a.cod_ndg
                                                           AND g.flg_workflow =
                                                                  3 --escludo eventuali destinazioni RIO non sbiancate o cancellate
                                                           AND NVL (
                                                                  flg_delete,
                                                                  0) = 0
                                                           AND NULLIF (
                          ...
                          (CASE
                              WHEN (   (    b.val_livello = 'DIREZIONE'
                                    ...
                                    OR (    b.val_livello = 'DIVISIONE' --                                    IN ('DIVISIONE', 'AREA',
                                        ...
                                        AND cod_stato IN ('OC', 'IN', 'RS') --v14.03.13
                                        AND 1 =
                                               (SELECT DISTINCT
                                                       CASE
                                                          WHEN val_liv_rischio_cli IN
                          ...
                          ------------------ ALERT_40 ---------------------
                          (CASE
                              WHEN ( (    b.val_livello = 'DIVISIONE' --                              IN ('DIVISIONE', 'AREA', 'REGIONE')--10/01/13 commento per escludere l'estensione a aree e regioni
                                             ...
                                             (SELECT DISTINCT 1
                                               ...
                                               WHERE     g.cod_abi_cartolarizzato =
                                                     ...
                                                     AND g.dta_richiesta
                                                            IS NOT NULL
                                                     AND g.flg_esito IS NULL
                                                     AND g.flg_storico = 0 --                                                     AND g.id_utente =  --14/02 Commentata condizione di join sull'utente. Da ripristinare.
                                                     --                                                            a.id_utente
                                                     AND (   NVL (
                                                                g.val_num_proroghe,
                                                                0) = 0
