/* Formatted on 21/07/2014 18:41:18 (QP5 v5.227.12220.39754) */
...
AS
   SELECT                                                    --VG 20140305 New
          ...
          --VG 20140624 Modifica logica calcolo colore dta_inserimento_delibera e flg_delibere
          A."ID_ALERT",
          A."COD_ABI",
          A."DESC_ISTITUTO",
          A."COD_NDG",
     ...
     FROM (SELECT a.ID_ALERT,
             ...
             FROM (SELECT 41 id_alert,
                          ...
                          NVL (FLG_PVISIONE_PRESIDIO, 'N')
                             FLG_PVISIONE_PRESIDIO,
                          CASE
                             --WHEN TRUNC (SYSDATE) - TRUNC (dta_conferma) <= g.val_current_green THEN  'V'
                             --WHEN TRUNC (SYSDATE) - TRUNC (dta_delibera) <= g.val_current_green THEN  'V'
                             WHEN   TRUNC (SYSDATE)
                             ...
                             --WHEN TRUNC (SYSDATE) - TRUNC (dta_delibera) BETWEEN g.val_current_green+ 1 AND g.val_current_orange THEN 'A'
                             WHEN   TRUNC (SYSDATE)
                             ...
                             --WHEN TRUNC (SYSDATE) - TRUNC (dta_delibera) >   g.val_current_orange THEN 'R'
                             WHEN   TRUNC (SYSDATE)
                                  - TRUNC (DTA_INSERIMENTO_DELIBERA) >
                          ...
                          NVL (
                             (SELECT DISTINCT 1
                                FROM t_mcres_app_delibere ss
                               WHERE     ss.cod_abi = s.COD_ABI
                                     AND ss.cod_ndg = s.COD_NDG
                                     AND SS.DTA_INSERIMENTO_DELIBERA >
                                            s.DTA_INSERIMENTO_DELIBERA),
                             0)
                             flg_delibere
