/* Formatted on 21/07/2014 18:44:35 (QP5 v5.227.12220.39754) */
...
AS
   WITH spese
        AS (SELECT --+ materialize
                                           ------ VG 20140610 Blocco invio sap
                  ROW_NUMBER () OVER (ORDER BY cod_autorizzazione) val_progressivo,
                  s.*
            ...
            WHERE     0 = 0
                  ...
                  AND s.cod_abi != '10637'
                  -- AP 20131128 modifica aggiunti tio proforma
                  AND s.cod_tipo_autorizzazione IN ('1', '5', '6')
   ...
   ***
   ...
   SELECT SUBSTR (s.cod_autorizzazione, 1, 4) cod_sap_societa,
          ...
          || RPAD (
                CASE
                   WHEN     s.val_numero_fattura IS NULL
                        AND s.cod_tipo_autorizzazione = '5'
                   THEN
                      'PROFORMA' || TO_CHAR (SYSDATE, 'yyyymmdd')
                   ELSE
                      NVL (s.val_numero_fattura, ' ')
                END,
                35,
                ' ')                                        -- nmero documento
          ...
          -- AP 20131128 modifica sottotipo da N a controllo proforma M
          || CASE WHEN s.cod_tipo_autorizzazione = '5' THEN 'M' ELSE 'N' END -- nota di credito
   ...
   ***
   ...
   ---AP 21031128 AGGIUNTA QUERY PER PROFORMA TIPOLOGIA IVA SA
   UNION ALL
     ---
     SELECT SUBSTR (s.cod_autorizzazione, 1, 4) cod_sap_societa,
            3 val_ordine_tipo_record,
            ...
            || RPAD (
                  ...
                  ' ')                                           -- conto GOGE
            || RPAD (' ', 24, ' ')                             -- elemento WBS
            || LPAD (NVL (SUM (f.val_imponibile_iva) * 100, 0), 13, '0') -- imponibile IVA
            || 'SZ' --'SA'                                                                                                                   -- codice SAP IVA
            || LPAD (0, 8, '0') --lpad(nvl(sum(f.val_aliquota_iva) * 100, 0), 8, '0')                                                         -- aliquota IVA
            || LPAD (0, 13, '0') --lpad(nvl(sum(f.val_importo_iva) * 100, 0), 13, '0')                                                        -- importo IVA
            || LPAD (NVL (SUM (f.val_importo_pos) * 100, 0), 13, '0') -- importo posizione
            || RPAD ('0', 13, '0')                                 -- quantità
            || RPAD ('0', 13, '0')                             -- prezzo netto
            || RPAD (' ', 13, ' ')                                -- numero EM
            || RPAD (' ', 99, ' ')                                   -- filler
   ...
   ***
   ...
   SELECT SUBSTR (s.cod_autorizzazione, 1, 4) cod_sap_societa,
          ...
          || RPAD (NVL (s.cod_importo_divisa, ' '), 3, ' ')          -- divisa
          || LPAD (NVL (f.val_totale_imponibile * 100, '0'), 20, '0') -- totale imponibile
          || LPAD (0, 20, '0') --lpad(nvl(s.importo_iva * 100, '0') , 20,'0')                                                -- totale IVA
          || LPAD (NVL (s.aliquota_cpa * 100, '0'), 20, '0')   -- aliquota CPA
          || LPAD (NVL (s.importo_cpa * 100, '0'), 20, '0')     -- importo CPA
          || LPAD (NVL (s.val_aliquota_ritenuta * 100, '0'), 20, '0') -- aliquota RA
          || LPAD (NVL (s.importo_ritenuta * 100, '0'), 20, '0') -- importo RA
