...
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_DATI_GEN_RV_HST
(
   COD_ABI,
   COD_NDG,
   COD_PROT_DELIBERA,
...
AS
   SELECT cod_abi,
     ...
     FROM (SELECT DISTINCT
                  ...
                  ---IN HOST LA RDV è SEMPRE E SOLO DI CASSA
                  NVL (d.val_rdv_pregr_fi, 0) AS val_rdv_pregr_firma,
                    --3 APRILE
                    (  NVL (d.val_rdv_qc_progressiva, 0)
                     + NVL (d.val_rdv_progr_fi, 0))
                  - (  NVL (d.val_rdv_qc_ante_delib, 0)
             ...
             FROM V_MCREI_APP_DETT_RAPP_RV_HST r,
                  t_mcrei_hst_delibere d,
                  t_mcrei_hst_delibere d2
