/* Formatted on 21/07/2014 18:40:07 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW MCRE_OWN.V_MCREI_APP_DETT_TRANSAZ_HST
(
   COD_ABI,
   COD_NDG,
...
AS
   SELECT DISTINCT
          ...
          d.val_rinuncia_deliberata AS val_rinuncia_deliberata,
          ...
          CASE
             ...
             THEN
                0
             ELSE
                  (NVL (d.val_rinuncia_totale, 0) * 100)
                / DECODE (d.cod_fase_delibera,
                          'IN', d.scsb_uti_cassa,
                          d.val_esp_lorda)
          END
             AS val_perc_rinuncia,
     ...
     FROM (SELECT d.*,
                  ...
                  r.scsb_acc_firma,
                  r.scsb_acc_cassa
             FROM t_mcrei_hst_delibere d, t_mcrei_app_pcr_rapp_aggr r
