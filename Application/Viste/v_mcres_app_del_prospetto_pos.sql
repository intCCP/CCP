/* Formatted on 21/07/2014 18:41:56 (QP5 v5.227.12220.39754) */
...
AS
   SELECT                   -- 20130719 VG cod_uo_pratica e cod_stato_delibera
     ...
     FROM (  SELECT D.COD_ABI,
                    ...
                    WHEN COD_DELIBERA IN ('NZ', 'FZ', 'RV', 'NS', 'AS') THEN 1
           ...
           GROUP BY D.COD_ABI,
                    ...
                    CASE
                       -- WHEN COD_DELIBERA IN ('NZ') THEN 0
                       WHEN COD_DELIBERA IN ('NZ', 'FZ', 'RV', 'NS', 'AS')
                       THEN
                          1
                       WHEN COD_DELIBERA IN ('TT', 'TP', 'TS')
                       THEN
                          2
                       WHEN COD_DELIBERA IN ('SN')
                       THEN
                          3
                       WHEN COD_DELIBERA IN ('RE')
                       THEN
                          4
                       WHEN COD_DELIBERA IN ('ES')
                       THEN
                          5
                       ELSE
                          -1
